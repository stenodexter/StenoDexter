      ...
      return db.transaction(async (tx) => {
        ...
        const [result] = await tx
          ...
          .values({
            ...
            userId,
            fullMistakes: Math.round(evaluation.fullMistakes),
            halfMistakes: Math.round(evaluation.halfMistakes),
            grossErrors: Math.round(evaluation.grossErrors * 2),
            errorStrokes: Math.round(evaluation.errorStrokes),
            totalStrokes: Math.round(evaluation.totalStrokes),
            netStrokes: Math.round(evaluation.netStrokes),
            grossWpm: Math.round(evaluation.grossWpm),
            netWpm: Math.round(evaluation.netWpm),
            accuracy: Math.round(evaluation.accuracy),
            netDph: Math.round(evaluation.netDph),
